...
//	private static final float CAMERA_HEIGHT = 10f;
	...
	public void render() {
		...
		spriteBatch.begin();
		//font.draw(spriteBatch, "abcdef", 100,100);
	...
	private void drawGameOver(){
		Bob bob = world.getBob();
		if(bob.isDying()){
		font.draw(spriteBatch, "Game over!", bob.getPosition().x * ppuX, (bob.getPosition().y + 3f) * ppuY);
		}
	}
	private void drawBoomEffect() {
		BoomEffect boom = world.getBoomEffect();
		
		if(boom.isBoom()){
		spriteBatch.draw(boom.textureRegion,((boom.getPosition().x - boom.SIZE/2) * ppuX),((boom.getPosition().y - boom.SIZE/2) * ppuY),boom.SIZE * ppuX, boom.SIZE * ppuY);
			//spriteBatch.draw(boom.textureRegion,(boom.getPosition().x * ppuX),(boom.getBounds().y * ppuY),boom.SIZE * ppuX, boom.SIZE * ppuY);
		}
//		boom.draw();
//		//spriteBatch.dr
		}
